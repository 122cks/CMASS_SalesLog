...
<body>
  <script>
    // Defensive guard: prevent automatic programmatic navigation on initial page load.
    // This prevents stray service-workers/old scripts or cached page code from immediately
    // navigating from `/` to `/front`. The guard restores original functions after
    // a short timeout or after the first user interaction (click/keydown).
    (function(){
      try{
        ...
        let restored = false;
        function restore(){ if (restored) return; restored = true; try{ window.location.assign = origAssign; window.location.replace = origReplace; window.open = origOpen; }catch(e){} }
        // temporarily override navigation methods
        window.location.assign = function(){ console.warn('[cmass guard] blocked assign during initial load'); };
        window.location.replace = function(){ console.warn('[cmass guard] blocked replace during initial load'); };
        window.open = function(){ console.warn('[cmass guard] blocked open during initial load'); };
        // restore on first user interaction
        const onUser = function(){ restore(); window.removeEventListener('click', onUser); window.removeEventListener('keydown', onUser); };
        window.addEventListener('click', onUser);
        window.addEventListener('keydown', onUser);
        // or restore after 1 second if no interaction
        setTimeout(restore, 1000);
      }catch(e){ console.warn('cmass guard init failed', e); }
    })();
  </script>
  <div class="wrap">
    <div class="card">
  ...
  <div class="version">build: 2025-10-22-06</div>
      <script>
  // Root PIN flow: submit name+PIN to /api/pin-check, store session marker, then redirect to front?staff=<token>
        (function(){
          ...
          // map select value -> Korean display name with title (same mapping as input.getStaffFromQuery)
          const displayMap = {
            'SongHoonjae': '송훈재 부장',
            'LimJunho': '임준호 차장',
            'ChoYounghwan': '조영환 부장'
          ...
          form.addEventListener('submit', async function(ev){
            ...
            // Local fallback PINs (useful for offline testing or when API is not available)
            // NOTE: These client-side PINs are embedded in the page and are not secure.
            const localPins = {
              'SongHoonjae': '8747',
              'LimJunho': '1203',
              'ChoYounghwan': '0686'
            ...
            // If the provided PIN matches a local mapping, accept it immediately and skip server check.
            if (localPins[token] && pin === localPins[token]) {
              try { sessionStorage.setItem('cmass_pin_authenticated', JSON.stringify({ staff: baseForPin, ts: Date.now() })); } catch(e){}
              window.location.href = '/front?staff=' + encodeURIComponent(token);
              return;
            }

            // Otherwise, fall back to server validation as before.
            try {
              ...
              if (resp.ok && j && j.ok) {
                ...
                const target = '/front?staff=' + encodeURIComponent(token);
