...
app.post('/save-meeting', async (req, res) => {
  try {
    ...
    const docData = {
      ...
      region: body.region || '',
      school: body.school || '',
      server_id: body.server_id || body.serverId || null,
      subjects: Array.isArray(body.subjects) ? body.subjects : (body.subjects ? [body.subjects] : []),
      ...
      ask: body.ask || body.requests || '',
      conversation: body.conversation || body.notes || '',
      delivery: body.delivery || body.deliveries || '',
      followUp: body.followUp || '',
      visitDate: body.visitDate || body.date || '',
      visitStart: body.visitStart || body.startTime || '',
      duration: body.duration || body.durationMin || 0,
      visitEnd: body.visitEnd || body.endTime || ''
    };
    docData.updatedAt = admin.firestore.FieldValue.serverTimestamp();
    // Persist as a single canonical record per staff|visitDate|region|school
    // Build a deterministic doc id so subsequent saves overwrite the previous
    const safe = (s) => encodeURIComponent(String(s || '')).replace(/\./g, '%2E');
    const staffKey = safe(docData.staff || '');
    const dateKey = safe(docData.visitDate || docData.visitDate || '');
    const regionKey = safe(docData.region || '');
    // prefer the explicit school field (display name) then school code if available
    const schoolKey = safe(docData.school || docData.schoolCode || '');
    const docId = docData.server_id ? String(docData.server_id) : `meeting|${staffKey}|${dateKey}|${regionKey}|${schoolKey}`;
    const ref = db.collection('meetings').doc(String(docId));
    // overwrite previous saved meeting for this key - keep it as the single final record
    await ref.set(Object.assign({}, docData, { savedAt: admin.firestore.FieldValue.serverTimestamp() }), { merge: false });
...
// POST /save-draft - save a draft keyed by staff|date|region|school
app.post('/save-draft', async (req, res) => {
  try {
    const body = req.body || {};
    const staff = (body.staff || '').toString().trim();
    const date = (body.date || body.visitDate || '').toString().trim();
    const region = (body.region || '').toString().trim();
    const school = (body.school || body.schoolCode || '').toString().trim() || (body.schoolName || '').toString().trim();
    if (!staff || !date) return res.status(400).json({ ok: false, msg: 'staff and date required' });
    // create deterministic doc id so subsequent saves overwrite
    const safe = (s) => encodeURIComponent(String(s || '')).replace(/\./g,'%2E');
    const docId = `draft|${safe(staff)}|${safe(date)}|${safe(region)}|${safe(school)}`;
    const payload = Object.assign({}, body || {});
    payload.updatedAt = admin.firestore.FieldValue.serverTimestamp();
    await db.collection('drafts').doc(docId).set(payload, { merge: true });
...
// GET /get-draft - fetch a draft by staff/date/region/school
app.get('/get-draft', async (req, res) => {
  try {
    ...
    const school = (req.query.school || req.query.schoolCode || '').toString().trim();
    if (!staff || !date) return res.status(400).json({ ok: false, msg: 'staff and date required' });
    const safe = (s) => encodeURIComponent(String(s || '')).replace(/\./g,'%2E');
    const docId = `draft|${safe(staff)}|${safe(date)}|${safe(region)}|${safe(school)}`;
    const snap = await db.collection('drafts').doc(docId).get();
    if (!snap.exists) return res.json({ ok: true, draft: null });
    const data = snap.data() || {};
    // Firestore Timestamps aren't JSON-serializable directly; convert if present
    if (data.updatedAt && data.updatedAt.toDate) data.updatedAt = data.updatedAt.toDate().toISOString();
    return res.json({ ok: true, draft: data, id: docId });
  } catch (err) {
