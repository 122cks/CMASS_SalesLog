...
(function(){
  ...
  // Accept either `date` or `visitDate` query param (meeting.html uses visitDate)
  const date = (qs('date') || qs('visitDate')) || (sessionStorage.getItem('cmass:last_report_date') || sessionStorage.getItem('cmass:last_date') || localStorage.getItem('cmass:last_date') || '');
  ...
  function renderEntry(e, idx){
    ...
    try{
      ...
      const edit = document.createElement('button'); edit.type = 'button'; edit.textContent = '수정'; edit.style.marginLeft = '8px'; edit.style.borderColor = '#d6e8f0'; edit.style.color = '#0a5'; edit.addEventListener('click', function(){
        try{
          // Inline editor for a single rendered entry (renderEntry used in other places)
          if (div.dataset.editing === '1') return;
          div.dataset.editing = '1';
          const editorWrap = document.createElement('div'); editorWrap.style.marginTop = '8px';
          const ta = document.createElement('textarea'); ta.style.width = '100%'; ta.style.minHeight = '120px'; ta.style.boxSizing = 'border-box';
          const existing = e.manualReportText && String(e.manualReportText).trim() ? String(e.manualReportText) : buildSingleEntryText(e, idx);
          ta.value = existing;
          editorWrap.appendChild(ta);
          const btnRow = document.createElement('div'); btnRow.style.marginTop = '6px'; btnRow.style.display = 'flex'; btnRow.style.gap = '8px';
          const save = document.createElement('button'); save.type='button'; save.textContent='저장'; save.style.borderColor='#cfe8d8'; save.style.color='#085';
          const cancel = document.createElement('button'); cancel.type='button'; cancel.textContent='취소'; cancel.style.borderColor='#d6d6d6';
          ...
          save.addEventListener('click', function(){
            try{
              // Persist manual text to report array under report key if possible,
              // otherwise save to the entry object and reload for simplicity.
              try{
                // attempt to find and update corresponding entry in the canonical array
                const reportKey = `report:${(staff||'').trim()}|${(date||'').trim()}`;
                const txt = localStorage.getItem(reportKey);
                if (txt){
                  const arr2 = JSON.parse(txt) || [];
                  // try to match by savedAt or school/start
                  let matchIdx = -1;
                  for (let i=0;i<arr2.length;i++){
                    const it = arr2[i]||{};
                    if ((it.savedAt||it._savedAt||'') && (it.savedAt||it._savedAt||'') === (e.savedAt||e._savedAt||'')) { matchIdx = i; break; }
                    if (it.school === e.school && (String(it.start||it.visitStart||it.startTime||'') === String(e.start||e.visitStart||e.startTime||''))) { matchIdx = i; break; }
                  }
                  if (matchIdx >= 0){ arr2[matchIdx].manualReportText = ta.value || ''; localStorage.setItem(reportKey, JSON.stringify(arr2)); window.location.reload(); return; }
                }
              }catch(err){ console.warn('save inline editor to canonical array failed', err); }
              // fallback: attach manualReportText to the entry object rendered in-memory and reload
              e.manualReportText = ta.value || '';
              window.location.reload();
            }catch(err){ console.warn('save inline edit failed', err); alert('저장 중 오류가 발생했습니다. 콘솔을 확인하세요.'); }
          });
          cancel.addEventListener('click', function(){ try{ div.removeChild(editorWrap); delete div.dataset.editing; }catch(e){ console.warn('cancel inline edit failed', e); } });
  ...
  try{
    ...
    if (summaryEl) {
      ...
      // fallback: insert into #main if present, otherwise log
      const mainEl = el('main');
      if (mainEl) mainEl.insertAdjacentHTML('afterbegin', summaryHtml);
      else console.warn('no #summary or #main to render summary');
    }
  }catch(e){ console.warn('failed to render summary', e); }

  // group by school
  const groupedEl = el('grouped');
  if (schoolNames.length === 0){
    if (groupedEl) groupedEl.innerHTML = '<p class="muted">해당 일자의 기록이 없습니다.</p>';
    else {
      const mainEl = el('main'); if (mainEl) mainEl.insertAdjacentHTML('beforeend', '<p class="muted">해당 일자의 기록이 없습니다.</p>');
    }
  } else {
    let html = '';
    ...
    try{
      ...
      if (txt){
        ...
        try{
          ...
          // per-school lines
          arr.forEach((it, idx) => {
            // If the user previously edited this entry's text manually, prefer that
            if (it.manualReportText && String(it.manualReportText).trim()){
              out += String(it.manualReportText) + '\n';
            } else {
              out += buildSingleEntryText(it, idx);
            }
          });
  ...
  // Build a single school's block text in the same format used by buildReportText
  function buildSingleEntryText(it, idx){
    try{
      ...
      out += '\n';
      return out;
    }catch(e){ console.warn('buildSingleEntryText failed', e); return '' }
  }
        ...
        try{
          ...
          editBtn.addEventListener('click', function(){
            try{
              ...
              // Inline editor: show a textarea prefilled with the generated block text
              if (node.dataset.editing === '1') return;
              node.dataset.editing = '1';
              const editorWrap = document.createElement('div'); editorWrap.style.marginTop = '8px';
              const ta = document.createElement('textarea'); ta.style.width = '100%'; ta.style.minHeight = '120px'; ta.style.boxSizing = 'border-box';
              // populate with manual text if present, otherwise generate from structured entry
              const existing = arr[foundIdx].manualReportText && String(arr[foundIdx].manualReportText).trim() ? String(arr[foundIdx].manualReportText) : buildSingleEntryText(arr[foundIdx], foundIdx);
              ta.value = existing;
              editorWrap.appendChild(ta);
              const btnRow = document.createElement('div'); btnRow.style.marginTop = '6px'; btnRow.style.display = 'flex'; btnRow.style.gap = '8px';
              const save = document.createElement('button'); save.type='button'; save.textContent='저장'; save.style.borderColor='#cfe8d8'; save.style.color='#085';
              ...
              save.addEventListener('click', function(){
                try{
                  arr[foundIdx].manualReportText = ta.value || '';
                  localStorage.setItem(reportKey, JSON.stringify(arr));
                  // refresh the page to reflect changes (keeps logic simple and consistent)
                  window.location.reload();
                }catch(e){ console.warn('save inline edit failed', e); alert('저장 중 오류가 발생했습니다. 콘솔을 확인하세요.'); }
              });
              cancel.addEventListener('click', function(){
                try{ node.removeChild(editorWrap); delete node.dataset.editing; }catch(e){ console.warn('cancel inline edit failed', e); }
              });
