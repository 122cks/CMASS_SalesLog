(function(){
  ...
  // Wait until `regions` array is populated (useful when restore runs before
  // CSV/regions have finished loading). Resolves true if regions available
  // within timeout, false otherwise.
  function waitForRegions(timeoutMs = 3000){
    return new Promise((resolve) => {
      try{
        if (regions && regions.length) return resolve(true);
        const start = Date.now();
        const iv = setInterval(() => {
          try{
            if (regions && regions.length) { clearInterval(iv); return resolve(true); }
  ...
  async function restoreDraftFromObject(d, key){
    if(!d) return;
    try{
      _isRestoring = true;
      // apply familiar fields (mirrors restoreDraft)
      if (staffEl) staffEl.value = d.staff||'';
      if (dateEl) dateEl.value = d.date||'';
      // Ensure region/school selects are populated before setting values
      try{
        if (d.region) {
          // wait briefly for regions to be loaded (CSV may be slow)
          const ok = await waitForRegions(3000);
          if (regionEl) regionEl.value = d.region || '';
          if (ok && d.region) populateSchoolSelect(d.region);
        } else if (regionEl) {
      ...
      if (schoolEl) {
        try{
          if (d.schoolCode) {
            const byVal = Array.from(schoolEl.options || []).find(o => o.value === d.schoolCode);
            schoolEl.value = byVal ? byVal.value : d.schoolCode;
          } else if (d.school) {
            const byVal = Array.from(schoolEl.options || []).find(o => o.value === d.school);
            const byText = Array.from(schoolEl.options || []).find(o => (o.textContent||'') === d.school);
            if (byVal) schoolEl.value = byVal.value; else if (byText) schoolEl.value = byText.value; else schoolEl.value = d.school || '';
          }
  ...
  try{ localStorage.setItem && localStorage.setItem('cmass:last_restore_values', JSON.stringify({ teacherVal, publisherVal, phoneVal, emailVal, requestsVal, notesVal, deliveriesVal })); }catch(e){}
  ...
  async function restoreDraft(){
    ...
    try{
      ...
      try{
        if (staffEl) staffEl.value = d.staff||'';
        if (dateEl) dateEl.value = d.date||'';
        // wait for region data if needed, then populate selects
        try{
          if (d.region) {
            const ok = await waitForRegions(3000);
            if (regionEl) regionEl.value = d.region || '';
  ...
  async function loadDraftForCurrentKey(){
    // Only restore when there's an exact local key match (staff|date|region|school).
    // Require all four key parts to be present (non-empty) so we don't prompt
    // when the page intentionally starts with an empty school or region.
    // Loose/fuzzy local matches (e.g. by date only or ignoring staff/school)
    // cause confusing prompts and are intentionally removed. Server draft
    // fetch remains as a non-interactive background attempt.
    try{
      if (_isRestoring) { debugLog('loadDraftForCurrentKey aborted: already restoring'); return; }
      // Only consider local exact-match if all four identifying fields are set
      const curStaff = (staffEl && (staffEl.value||'').trim()) || '';
      const curDate = (dateEl && (dateEl.value||'').trim()) || '';
      const curRegion = (regionEl && (regionEl.value||'').trim()) || '';
      const curSchool = (schoolEl && (schoolEl.value||'').trim()) || '';
      if (!curStaff || !curDate || !curRegion || !curSchool) {
        debugLog('loadDraftForCurrentKey: skipping local prompt because one or more key parts are empty', { staff: !!curStaff, date: !!curDate, region: !!curRegion, school: !!curSchool });
      } else {
        const key = buildKey();
        if (key) {
          const txt = localStorage.getItem(key);
          if (txt){
            try{ debugLog('loadDraftForCurrentKey: found exact local draft', key); const d = JSON.parse(txt); if(d){ restoreDraft(); renderSavedDraft(d); } }catch(e){ debugLog('loadDraftForCurrentKey: local parse error', e && e.message); }
            return;
          }
        }
      }

      // No exact local draft — attempt to fetch a server draft silently and
      // merge it into localStorage, but do not prompt the user based on
      // loose local matches.
      debugLog('loadDraftForCurrentKey: no exact local draft found, fetching server draft', key);
      const serverDraft = await fetchDraftFromServer();
      if(serverDraft){
        debugLog('loadDraftForCurrentKey: serverDraft received');
        try{ if(!serverDraft._savedAt) serverDraft._savedAt = new Date().toISOString(); }catch(e){}
        try{ localStorage.setItem(key, JSON.stringify(serverDraft)); }catch(e){}
        _isRestoring = true; try{ restoreDraft(); }finally{ _isRestoring = false; }
        renderSavedDraft(serverDraft);
        try{ showRestoreNotice(key); }catch(e){}
      }
  ...
  function checkAndPromptRestoreOnCurrentKey(){
    try{
      ...
      // Only prompt when all four identifying fields are set and the stored
      // draft exactly matches those values. This prevents prompts when the
      // school/region are unset or when a partially matching draft exists.
      const curStaff = (staffEl && (staffEl.value||'').trim()) || '';
      const curDate = (dateEl && (dateEl.value||'').trim()) || '';
      const curRegion = (regionEl && (regionEl.value||'').trim()) || '';
      const curSchool = (schoolEl && (schoolEl.value||'').trim()) || '';
      if (!curStaff || !curDate || !curRegion || !curSchool) {
        debugLog('checkAndPromptRestoreOnCurrentKey: not prompting because one or more key parts are empty', { staff: !!curStaff, date: !!curDate, region: !!curRegion, school: !!curSchool });
        return;
      }
      const key = buildKey();
      if(!key) return;
      ...
      if(txt){
        try{
          ...
          // Double-check the parsed draft fields exactly match current UI values
          const ds = (obj && (obj.staff||'').trim()) || '';
          const dd = (obj && (obj.date||'').trim()) || '';
          const dr = (obj && (obj.region||'').trim()) || '';
          const dscho = (obj && (obj.school||'').trim()) || '';
          if (ds === curStaff && dd === curDate && dr === curRegion && dscho === curSchool) {
            showAskRestoreModal(key, obj); return;
          } else {
            debugLog('checkAndPromptRestoreOnCurrentKey: local draft exists but does not exactly match current UI values', { draft: { staff: ds, date: dd, region: dr, school: dscho }, current: { staff: curStaff, date: curDate, region: curRegion, school: curSchool } });
            return;
          }
        }catch(e){ showAskRestoreModal(key, null); return; }
      }
      // No exact-match local draft found — do not perform fuzzy local matching.
      // We only prompt when staff|date|region|school all match exactly.
  ...
  const pSchool = params.has('school') ? (params.get('school') || '') : '';
    ...
    if (pRegion && regionEl) {
    try {
      // When applying URL params programmatically during initialization
      // we must avoid triggering autosave handlers which would overwrite
      // any existing saved draft. Set _isRestoring while we set values and
      // dispatch change events so change listeners can bail out.
      _isRestoring = true;
      regionEl.value = pRegion;
      try{ sessionStorage.setItem('cmass:last_region', pRegion); }catch(e){}
    ...
    try {
      ...
      try{ sessionStorage.setItem('cmass:last_school', (schoolEl.selectedOptions && schoolEl.selectedOptions[0] && schoolEl.selectedOptions[0].textContent) || schoolEl.value || pSchool); }catch(e){}
