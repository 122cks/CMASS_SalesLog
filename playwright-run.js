...
(async () => {
  ...
  // Run headless in CI environment; slowMo/visible window not needed here.
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();
  // ensure a large viewport so buttons are in-layout and bounding boxes are available
  ...
  try {
    ...
    // wait for region/school UI to appear. Try multiple selectors (some deployments
    // render buttons, others render container wrappers or selects). Try each with a
    // modest timeout so the total wait is bounded but robust across variants.
    const waitSelectors = ['#regionButtons .grid-button', '#regionButtons', '#schoolButtons .grid-button', 'button.grid-button', 'select#regionSelect', 'select#schoolSelect'];
    ...
    if (!foundSelector) throw new Error('page UI selectors not found (region/school controls)');
